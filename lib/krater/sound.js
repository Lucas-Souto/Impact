...
class SoundManager
{
	...
	constructor()
	{
		...
		// KG.normalizeVendorAttribute(window, 'AudioContext');
		...
		if(Sound.enabled && Sound.useWebAudio)
		{
			...
			KG.system.canvas.addEventListener('touchstart', this.boundWebAudioUnlock, false);
			KG.system.canvas.addEventListener('mousedown', this.boundWebAudioUnlock, false);
		}
	}
	
	unlockWebAudio()
	{
		KG.system.canvas.removeEventListener('touchstart', this.boundWebAudioUnlock, false);
		KG.system.canvas.removeEventListener('mousedown', this.boundWebAudioUnlock, false);
	...
	loadWebAudio(path, multiChannel, loadCallback)
	{
		...
		const realPath = KG.prefix + path.replace(/[^\.]+$/, this.format.ext) + KG.nocache;
	...
	loadHTML5Audio(path, multiChannel, loadCallback)
	{
		...
		if(loadCallback)
		{
			...
			// load callback
			if(KG.ua.mobile) setTimeout(() => loadCallback(path, true, null), 0);
			else
			{
...
class Music
{
	...
	add(music, name)
	{
		...
		const track = KG.soundManager.load(path, false);
...
class Sound
{
	...
	load(loadCallback)
	{
		...
		if(KG.ready) KG.soundManager.load(this.path, this.multiChannel, loadCallback);
	...
	play()
	{
		...
		this.currentClip = KG.soundManager.get(this.path);
		this.currentClip.loop = this._loop;
		this.currentClip.volume = KG.soundManager.volume * this.volume;
...
class WebAudioSource
{
	...
	constructor()
	{
		this.gain = KG.soundManager.audioContext.createGain();
		this.gain.connect(KG.soundManager.audioContext.destination);
	...
	play()
	{
		...
		const source = KG.soundManager.audioContext.createBufferSource();
